...
namespace SharpSid
{
    public class Voice
    {
        ...
        public int output
        {
            get
            {
                if (!muted)
                {
                    ...
                    return ((wave.output - wave_zero) * envelope.envelope_counter + voice_DC);
                }
                else
                {
                    return 0;
        ...
        {
            wave.SaveToWriter(writer, v0, v1, v2);
            envelope.SaveToWriter(writer);
            writer.Write(muted);
            writer.Write(wave_zero);
            writer.Write(voice_DC);
        }
        // deserializing
